 ...
 *  2016/3/28 16:23:34
 *  邱老师 
 *  1. 输出目录如果不存在，应该自动创建(已完成)
 *  2. 当命令行参数个数为0时，就默认打印帮助信息(已完成)
 *  3. 当使用-r参数时，没有正确判断输出目录是否属于源目录的子目录，还是把输出目录当成了源码目录进行了处理(已完成)
 *  4. 当使用-r参数时，输出目录下也要对应创建子目录，如源目录\temp\22对应输出输出目录\out\2(已完成)
 * 
 */
...
//				System.out.println(targetpath.getParentFile().getName());
//			}
//			try
//			{
//				targetpath.createNewFile();
//				System.out.println(targetpath.createNewFile());
//			}
//			catch(Exception e)
//			{
//				System.out.println("\n目标文件创建异常！！！");
...
//					filelist.add(files[i] , files[i].toPath().getParent());
				else
					continue;
			}
		}
...
//						System.out.println(file4.getParentFile().getName());
					}
					try
					{
						file4.createNewFile();
